...
#define BETA_CUR_D1_ADDR 521
#define MU_CUR_D1_ADDR 523
#define SIGMA_CUR_D1_ADDR 525
// TODO: Add here the addresses for other features
...
int main(void) {
    ...
    // SVM parameters
    // TODO: change these values to the ones of your SVM, can be generated with the MATLAB script 'conversion_svm.m'
    const uint32_t scale_msb = 0b0000000000;
    const uint32_t scale_lsb = 0b00000000000000000000000000111000;
    const uint32_t bias_msb = 0b1111111111;
    const uint32_t bias_lsb = 0b11111111111111111110001011101000;
    const uint32_t betas_msb = 0b1111111111;
    const uint32_t betas_lsb = 0b11111111111111111111111101010010;
    const uint32_t mus_msb = 0b0000000000;
    const uint32_t mus_lsb = 0b00000000001101101100011000111010;
    const uint32_t sigmas_msb = 0b0000000000;
    const uint32_t sigmas_lsb = 0b00000000001000101110011001001111;
    ...
    while (1) {
        ...
        case INIT:
            ...
            // TODO: change this if different features are used
            // Watch out for the order of the features
        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * BETA_CUR_D1_ADDR, betas_lsb);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (BETA_CUR_D1_ADDR + 1), betas_msb);

        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * MU_CUR_D1_ADDR, mus_lsb);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (MU_CUR_D1_ADDR + 1), mus_msb);

        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * SIGMA_CUR_D1_ADDR, sigmas_lsb);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (SIGMA_CUR_D1_ADDR + 1), sigmas_msb);
