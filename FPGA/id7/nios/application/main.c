...
#define BETA_ENE_D3_ADDR 527
#define MU_ENE_D3_ADDR 529
#define SIGMA_ENE_D3_ADDR 531
...
int main(void) {
    ...
    const uint32_t scale_lsb = 0b00000000000000000000101110001010;
    const uint32_t bias_msb = 0b1111111111;
    const uint32_t bias_lsb = 0b11111111111111111111001101111111;

    // Ene D2 and then Ene D3
    const uint32_t betas_msb[2] = {0b1111111111, 0b0000000000};
    const uint32_t betas_lsb[2] = {0b11111111111111111101001011000001, 0b00000000000000000000100001001101};
    const uint32_t mus_msb[2] = {0b0000000000, 0b0000000000};
    const uint32_t mus_lsb[2] = {0b00000000000000111100101001100000, 0b00000000000110110100010111100001};
    const uint32_t sigmas_msb[2] = {0b0000000000, 0b0000000000};
    const uint32_t sigmas_lsb[2] = {0b00000000000000101100100101111001, 0b00000000000101100101001010011101};
    ...
    while (1) {
        ...
        case INIT:
        	...
        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * STOP_ADDR, 1);
        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * STOP_ADDR, 0);

            // Send SVMs to the predictor
        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * BIAS_ADDR, bias_lsb);
        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (BIAS_ADDR + 1), bias_msb);

        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * SCALE_ADDR, scale_lsb);
        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (SCALE_ADDR + 1), scale_msb);

        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * BETA_ENE_D2_ADDR, betas_lsb[0]);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (BETA_ENE_D2_ADDR + 1), betas_msb[0]);

        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * MU_ENE_D2_ADDR, mus_lsb[0]);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (MU_ENE_D2_ADDR + 1), mus_msb[0]);

        	IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * SIGMA_ENE_D2_ADDR, sigmas_lsb[0]);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (SIGMA_ENE_D2_ADDR + 1), sigmas_msb[0]);


			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * BETA_ENE_D3_ADDR, betas_lsb[1]);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (BETA_ENE_D3_ADDR + 1), betas_msb[1]);

			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * MU_ENE_D3_ADDR, mus_lsb[1]);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (MU_ENE_D3_ADDR + 1), mus_msb[1]);

			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * SIGMA_ENE_D3_ADDR, sigmas_lsb[1]);
			IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (SIGMA_ENE_D3_ADDR + 1), sigmas_msb[1]);
        ...
        case SEND_VAL:
            ...
            for (int i = 0; i < 512; i++) {
                ...
                IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * i, (uint32_t) value);
            }

            // Send order of starting predictor
            IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * START_ADDR, 1);
            ...
            IOWR_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * START_ADDR, 0);
        ...
        case WAIT_DONE:
        	...
        	has_done = IORD_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * FINISHED_ADDR);
            if (has_done == 1) {

                // Read the output value from the predictor, needs two 32-bit reads
                uint32_t lsb = IORD_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * RESULT_ADDR);
                uint64_t msb = (uint64_t) IORD_32DIRECT(PREDICTOR_INTERFACE_0_BASE, 4 * (RESULT_ADDR + 1));
